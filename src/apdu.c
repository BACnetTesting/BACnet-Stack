...
/* APDU Timeout in Milliseconds */
  // BTC - has to be 10000 if not modifiable. Should be 6000 if modifyable todonext2, 
 ...
 * @ingroup MISCHNDLR
 *
 * @param src [in] The BACNET_ROUTE of the message's source.
    ...
    uint16_t apdu_len)
{
    ...
    if (apdu) {
        ...
        case PDU_TYPE_CONFIRMED_SERVICE_REQUEST:
                ...
                apdu_len, &service_data, &confirmed_service_choice, &service_request,
                &service_request_len);
            if (apdu_confirmed_dcc_disabled(confirmed_service_choice)) {
                /* When network communications are completely disabled,
                   only DeviceCommunicationControl and ReinitializeDevice APDUs
                   shall be processed and no messages shall be initiated. */
                break;
            ...
            if ((confirmed_service_choice < MAX_BACNET_CONFIRMED_SERVICE) &&
                ...
                Confirmed_Function[confirmed_service_choice](service_request,
        ...
        case PDU_TYPE_UNCONFIRMED_SERVICE_REQUEST:
            ...
            if (apdu_unconfirmed_dcc_disabled(unconf_service_choice)) {
            ...
            if (unconf_service_choice < MAX_BACNET_UNCONFIRMED_SERVICE) {
                if (Unconfirmed_Function[unconf_service_choice])
                    Unconfirmed_Function[unconf_service_choice](service_request,
        ...
        case PDU_TYPE_SEGMENT_ACK:
            ...
            /* FIXME: what about a denial of service attack here?
               ...
               // EKH: Go back to Karg. Where do wwe get invoke_id from?
               // tsm_free_invoke_id(invoke_id);
#endif
            break;
...
#if ( BACNET_CLIENT == 1 )
            // todo 3 - do we need to handle possible responses to confirmed events here?
            invoke_id = apdu[1];
            reason = apdu[2];
            if (Reject_Function)
