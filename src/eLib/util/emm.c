...
// this will check multiple pointers for validity, and if any invalid, free
// all remaining
...
#if defined ( OS_LAYER_LINUX ) || defined ( OS_LAYER_WIN )
    // this is just a max size safety check
#if ( BACNET_SVC_COV_B == 1 ) || ( BACNET_USE_OBJECT_SCHEDULE == 1 )
    uint maxSize = 60000;   // this is a ridiculous size for a single object. Cull oversize structures
#else
    uint maxSize = 0x0C00;
#endif
#else
#error - define OS_LAYER_XXX
#endif // OS_LAYER_xxx
...
void emm_free(void *mptr)
{
  ...
  uint8_t *ptr = (uint8_t *) mptr - sizeof ( EMM_FENCE ) ;
    ...
    free(mptr);
    // todo 3 - RPM bug here (Extreme RPM stress test)
#endif  // BAC_DEBUG
}
