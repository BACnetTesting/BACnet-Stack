    ...
    char *argv[])
{
    if (argc < 2)
    {
        printf("Specify your local IP address on command line (or in debugger settings).\n" );
        exit(1);
    } else {
    ...
    do
    {
        ...
        case 'e':
            ...
            dbMessage(DBD_Config|DBD_UI, DB_NOTE, "Debug level set to %d", dblev);
            break;
        ...
        case 'v':
            dbMessage(DBD_Config | DBD_UI, DB_ALWAYS, "%s", "Version: " APPLICATION_VERSION_TEXT);
            exit(0);
            break;
        ...
        case 's':
            simulationCount = (unsigned) atoi(optarg);
            dbMessage(DBD_Config, DB_NOTE, "Simulating %d Virtual Devices", simulationCount);
            break;
