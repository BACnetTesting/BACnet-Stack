...
void bits_board_led_init (
		void )
{
	// todo 1 - Make a method to allow multiple functions to be assigned to a given LED

	// blue. Unfortunately there are only 2 LEDs on the Dev board.
	...
	boardLeds[BOARD_LED_RED].board_led_function = BOARD_LED_FUNCTION_DATALINK_MSTP_RX ;
	boardLeds[BOARD_LED_RED].board_led_function_ix = 0 ;

	// leave green uninitialized for now, mainly because it doesn't work out the box.
}


	// todo - add an index parameter for multiple rx connections,
void led_rx_on_interval(
    uint16_t interval_ms)
{
	bits_board_led_pulse_indexed( BOARD_LED_FUNCTION_DATALINK_MSTP_RX, 0, 10 ) ;
}
...
void led_rx_error(
    uint16_t interval_ms)
{
	bits_board_led_pulse_indexed( BOARD_LED_FUNCTION_DATALINK_MSTP_ERR, 0, 100 ) ;
}
...
void led_tx_on_interval(
    uint16_t interval_ms)
{
	// Not enough LEDs available on this board, unfortunately
}
