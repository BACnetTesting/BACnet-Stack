...
// #include "bacnet/bits/util/llist.h"
// #include "led.h"
...
{
    // todo - this is not actually calculating anything, let alone rollover...
    panic();
...
bool matchMSTP(void *llitem, void *matchdata)
{
    ...
    panic();
    // return pkt->dlcb->expectingReply ;
    return false;
    ...
    mstp_port_struct *mstp_port)
{
    ...
    if (testSetup != 0)
    {
        ...
        case 'H':
            if (mstp_port->master_state == MSTP_MASTER_STATE_USE_TOKEN)
            {
                ...
                PrepareTriggers();
                dlmstp_Send_Frame(mstp_port, (uint8_t *)"UUUU", 4);   // Nmin_octets
...
#ifdef AUTOMAC
        // dlmstp_automac_handler(mstp_port);
        //if (mstp_port->This_Station != 255) {
        //    mstp_port->Next_Station = automac_next_station(mstp_port->This_Station);
        //    if (mstp_port->Next_Station == 255) {
    ...
    //{
                ...
                if ((mstp_port->DestinationAddress ==
                    ...
                    switch (mstp_port->FrameType) {
                        ...
                        case FRAME_TYPE_POLL_FOR_MASTER:
                            ...
                            // todo 0 - karg has additional code here
                            panic();
        ...
        } else if (SilenceTimer(mstp_port) >=
                Tno_token) {
                /* LostToken */
...
//            priorFrameType = FRAME_TYPE_POLL_FOR_MASTER;
        ...
        if (SilenceTimer(mstp_port) <= Tusage_timeout) {
            // todo 0 ekh earlier code had >= here. Review
        ...
        if (SilenceTimer(mstp_port) < my_timeout) {
            // todo1 ekh earlier code had >= here. Review
                if (mstp_port->EventCount > Nmin_octets) {
                    /* SawFrame */
            ...
            // use that, else we risk a collision.  BMDA candidate. todo 1
        ...
        if (ll_GetCount(&mstp_port->outputQueueMSTP))
        {
            pkt = (struct mstp_pdu_packet *)ll_Pluck(&mstp_port->outputQueueMSTP, NULL, matchMSTP);
                /* Reply */
            ...
            Treply_delay)
        {
            ...
            default:
                mstp_port->ReceivedValidFrame = false;
                // todo 0 - karg does not have an idle here....
        mstp_port->master_state = MSTP_MASTER_STATE_IDLE;
                break;
        }
#endif
