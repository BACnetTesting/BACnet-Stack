...
void InitBACnetDataStructures(void);
...
// in eLib void msSleep(uint16_t ms);
...
// in tokenize.h char **Tokenize(char *line, const char divider, int *outCount);

bool bits_FileExists(const char* name);

void bits_Init_Service_Handlers(void);

void bits_KeyInput(char key);
uint8_t bits_getch(void);
uint8_t bits_getch_toupper(void);
