    ...
    BACNET_READ_ACCESS_DATA * read_access_data)
{
    ...
    len =
        (int) memcopy(&apdu[0], &apdu_temp[0], (uint16_t) apdu_len, (uint16_t) len,
                      (uint16_t) max_apdu);
    if (len == 0) {
        return 0;
    }
    ...
    while (rpm_object) {
        ...
        len =
            (int) memcopy(&apdu[0], &apdu_temp[0], (uint16_t) apdu_len,
                          (uint16_t) len, (uint16_t) max_apdu);
        if (len == 0) {
            return 0;
        }
        apdu_len += len;
    ...
    BACNET_RPM_DATA * rpmdata)
{
    ...
    BACNET_UNSIGNED_INTEGER array_value = 0;   /* for decoding */
