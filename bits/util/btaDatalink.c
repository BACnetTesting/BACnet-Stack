...
void SendBTApayload(uint8_t *payload, const int sendlength)
{
    ...
    uint8_t *stringPayload = (uint8_t *) emm_smalloc('1', MX_BTA_BUFFER);
    ...
    for (int i = 0; i < sendlength; i++) {
        ...
        bits_memcpy(&stringPayload[oPtr], hexa, 3);
        oPtr += 3;
    ...
    bits_memcpy(&stringPayload[oPtr], "\r\n", 2);
    oPtr += 2;
...
//     if (!CDC_Transmit_Ready()) return;
    ...
    // we can send 
    btaLB *btalb = (btaLB *) ll_Dequeue(&btaOutputQueueCB);
