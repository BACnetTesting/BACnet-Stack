...
void ParseFDRfile(const char *fileName)
{
    ...
    if (configFile == NULL)
    {
        tldReport(TLDP_Configuration_Error, "FDR config filename %s does not exist. Ignoring Foreign Device Registration.\n", fileName);
        return;
    }
    else
    {
        tldReport(TLDP_Configuration_Message, "******* BACnet Gateway FDR Settings *******\n");
        ...
        while (!feof(configFile)) // read through the file until End of File
        {
            ...
            if (tokens != NULL && strlen ( tokens[0] ) > 9)
            {
                if (!StringTo_IPEP(&ipep, tokens[0]))
                {
                    tldReport(TLDP_Configuration_Error, "Error, could not parse: %s\n", line);
                    // panic();
                    // emm_free(tokens);
                    continue;
                }
                ...
                char tbuf[30];
                tldReport(TLDP_Configuration_Message, "Will register with BBMD %s\n", IPEP_ToString( tbuf, &ipep) );
