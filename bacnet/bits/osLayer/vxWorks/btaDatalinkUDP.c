...
static bool FindBTAport(void)
{
    for (int i = 0; i < sizeof(btaPorts) / sizeof(uint16_t); i++) {
        ...
        // that's right folks, vxWorks does not like the const here
        int status = bind(sock_fd, (struct sockaddr *)&sin, sizeof(struct sockaddr));
        if (status == 0) {
            dbTraffic( DBD_Config, DB_UNUSUAL_TRAFFIC, "BTA listen port is %d\n", btaPorts[i]);
    ...
    socklen_t sinlen)
{
    ...
    case ClientAlive:
        ...
        if (!isEqualIPEP(sinIPEP, &btaClientIPEP)) {
            ...
            int namelen = sizeof(btaLocalIPEP);
...
{
	// we know we will need emm, queues for buffers etc.
	emm_init();
...
// i.e. are we initialized, and have we heard from BTA client (recently)
static bool BTA_Ready(void)
{
