...
//    61077,
//    47824,      // 0xBAD0
//    33330,      // Added a few more... can easily see 2, 3 apps running on one machine, plus a few spare for other collisions
//    44440,
//    55550,
//    61111
//};
...
//static struct sockaddr_in btaIPEP;
//static int sock_fd;
...
//        if ( ! isEqualIPEP ( sinIPEP, &btaIPEP ) ) {
...
//            int err = getsockname(sock_fd, (struct sockaddr *) &ourAddress, &namelen);
//            if (err) {
//                printf("getcockname error\r\n");
//            }
//            else {
//                printf("local = %s:%d\n\r", IPAddr_ToString(tbuf, &ourAddress.sin_addr), ntohs(ourAddress.sin_port ));
//                // clientIsKnown = true;
//            }
//
...
//                panic();
//#endif  
//                // let's not crash the CPU if we continuously err... (todo1 restart connection process)
//                msSleep(100);
//            }
//            else {
...
void SendBTAmstpFrame( const uint8_t portId, const bool tx, const uint8_t *frame, uint16_t data_len )
{
  ...
  // if ( ! BTA_Ready() ) return ;
    ...
    int datalen = BITS_MIN(data_len, MX_BTA_BUFFER - BX_DATAOFFSET);
