...
static void Init_BACnetIdle_Thread(void)
{
    ...
    if (rcode == -1L) {
        dbTraffic(DBD_ALL, DB_ERROR, "Failed to create thread");
    }
#else
    int rcode;
    pthread_t threadvar;
...
static void TickBACnetDevice(void)
{
    ...
    // bad- cancels our timer. remove last_seconds = current_seconds ;
    ...
    if (elapsed_seconds) {
        ...
        // Note: These two functions will be handled by the datalink threads in the Multiple
        // datalink model.
...
bool TickBACnet(void)
{
    ...
    // todo1 bits_mutex_lock(stackLock);
    TickBACnetDevice();

    // todo1 bits_mutex_unlock(stackLock);
    return true;
}
