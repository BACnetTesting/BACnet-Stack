...
BOARD_LED_CB	boardLeds[MX_BOARD_LEDS] ;

static BOARD_LED_CB *bits_board_led_find_indexed ( BOARD_LED_FUNCTION func, uint index )
{
...
//void bits_board_led_init ( LED_CB *ledCB, GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, uint16_t msPulseTime )
//{
...
void bits_board_led_idle( uint milliSecs )
{
	for( int i=0; i<sizeof(boardLeds) / sizeof(BOARD_LED_CB); i++)
	{
		...
		void )
{
	...
	// and then flash them all once at startup
	for (int i=0; i<MX_BOARD_LEDS; i++) {
		boardLeds[i].timeRemaining = 10000 ;
		bits_board_led_assert( &boardLeds[i], true ) ;
	}
}
