...
// 2019.02.24 EKH: this does not need to belong in the datalink layer, moving up to here.
// Timer that indicates line silence timer
...
// Timer that reset line silence timer
    ...
    uint16_t data_len)          /* number of bytes of data (up to 501) */
{
    ...
    if (txShortFrame) {
        // prints("<M- Sending short frame\r\n");
        SendBTAmessage("Sending short frame 2");
        txShortFrame = false;
        return 3;
    }
...
//    mstp_port->DestinationAddress != 121) return;
    ...
    static uint heartbeat = 100 ;
    if (!heartbeat) {
        // prints("<M- Heartbeat\r\n");
        SendBTAmessage("Hearbeat");
        heartbeat = 1000;
    }
    else {
        heartbeat--;
    ...
    uint16_t data_len)                                  /* number of bytes of data (up to 501) */
{
    ...
    if (len < 8) {
        ...
        SendBTAhexdump("Short Frame", mstp_port->OutputBuffer, len);
    }
    else {
    ...
    case MSTP_RECEIVE_STATE_IDLE:
        ...
        if (mstp_port->ReceiveError == true) {
            ...
            if (mstp_port->DataRegister == 0x55) {
                ...
                // bits_ScopeTrigger();
                // prints("Not 0x55\r\n");
                // todo 0 - OK, this successfully detected the fact that the BRCM is using 0xFF to close off on comms
                // perhaps we should log the fact?
            }
    ...
    volatile struct mstp_port_struct_t * mstp_port)
{
    ...
    static int oldState = MSTP_MASTER_STATE_INITIALIZE;
    if (master_state != oldState) {
        SendBTAtransition(FMST_Master, (uint8_t) oldState, (uint8_t) master_state);
        oldState = master_state;
    }
#endif
    ...
    volatile struct mstp_port_struct_t *mstp_port)
{
    ...
    mstp_port->Index = 0;
    mstp_port->This_Station = 53 ;                                  // todo - get from flash
    mstp_port->Next_Station = mstp_port->This_Station;
    mstp_port->Poll_Station = mstp_port->This_Station;
    mstp_port->ReceivedInvalidFrame = false;
    mstp_port->ReceivedValidFrame = false;
...
#if ( MSTP_DIAG_TRANSITION == 1)
void SendBTAtransition(FSMT sm, uint8_t oldState, uint8_t newState) // , const char *message)
{
    ...
    PrepareBTAheader(BTApt_MSTPtransition, outbuf);

    outbuf[22] = sm;
    outbuf[23] = oldState;
    outbuf[24] = newState;
    outbuf[25] = 0 ; // not sure what this is...

    SendBTApayload(outbuf, 25);
}
#endif
