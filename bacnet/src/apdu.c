        ...
        for (i = 0; i < MAX_BACNET_CONFIRMED_SERVICE; i++) {
            if (confirmed_service_supported[i] == service_supported) {
                ...
                if (Confirmed_Function[i] != NULL) {
                	// supporting all services (for now) todo2
//#if ( BAC_ROUTING == 1 )
    ...
    if (apdu) {
            ...
            case SERVICE_CONFIRMED_REQUEST_KEY:
                if (Confirmed_ACK_Function[confirmed_service_choice] != NULL) {
                    ((confirmed_simple_ack_function)
                        Confirmed_ACK_Function[confirmed_service_choice]) (src,
                            invoke_id);
                }
        ...
        case PDU_TYPE_COMPLEX_ACK:
            ...
            case SERVICE_CONFIRMED_AUTHENTICATE:
                ...
                // 2016.11.03 
                panictodo(3);
                // again, this free tried to free an invoke based on a returning message regarding READ_MODEL_NAME.
                // e.g. a very late reply, and TMS has already timed it out...? (while stalled in BTC at 8.4.2-a where I singlestepped 
                // through:  BACnetProperty originalTS = testObject.ReadProperty(PROPERTY_ID.EVENT_TIME_STAMPS); )
                tsm_free_invoke_id(pDev, invoke_id);
                break;
            default:
                break;
        ...
        case PDU_TYPE_SEGMENT_ACK:
            ...
            /* FIXME: what about a denial of service attack here?
               ...
               // EKH: Go back to Karg. Where do wwe get invoke_id from? // todo 2
                // todo2 
                // tsm_free_invoke_id(pDev, invoke_id);
#endif
            break;
