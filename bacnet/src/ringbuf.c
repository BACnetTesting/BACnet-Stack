...
bits_mutex_define(mutexRingbuf);
...
volatile uint8_t *Ringbuf_Peek(RING_BUFFER const *b)
{
    ...
    bits_mutex_unlock( mutexRingbuf ) ;
    ...
    uint8_t * data_element)
{
    ...
    if (b && data_element) {
        ...
        if (!Ringbuf_Full(b)) {
            ...
            b->head++;
            Ringbuf_Depth_Update(b); // todo1 - this is new..
            status = true;
        }
		else
		{
            // todo1 - remove for delivery
            // todo1 - if the ringbuf is 8, we get overflows in BTC on discovery.. fix this, also add instrumentation
			dbTraffic(DBD_ALL, DB_ERROR, "Ringbuf full %p Size:%d %d %d Count:%u\n",
                   b,
                   b->element_count,
                   b->head,
                   b->tail, Ringbuf_Count(b) ) ;
		}
    }
	else
	{
        // todo1 - remove for delivery
        SendBTAmessage("Other problem");
    }
