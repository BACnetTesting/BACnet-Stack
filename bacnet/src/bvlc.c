    ...
    for (i = 0; i < MAX_FD_ENTRIES; i++) {
        if (FD_Table[i].valid) {
            if ((FD_Table[i].dest_address.s_addr == sin->sin_addr.s_addr) &&
                ...
                status = true;
                FD_Table[i].time_to_live = time_to_live_seconds;
                ...
                FD_Table[i].seconds_remaining = time_to_live_seconds + 30;
                break;
            }
        }
    }
    ...
    const uint16_t mtu_len)
{
    ...
    bip_ipAddr_port_to_bacnet_mac(&dummyDstMAC, bvlc_dest.sin_addr.s_addr, bvlc_dest.sin_port);

    // and in this case, the dst address is ourselves

    bip_ipAddr_port_to_bacnet_mac(&dummySrcMAC, ourAddr, ourPort ) ;
    ...
    uint16_t npdu_length)
{
    ...
    mtu_len =
        (uint16_t)bvlc_encode_forwarded_npdu(&mtu[0], max_npdu, sin, npdu,
            npdu_length);
...
//    char tstring[100];
    ...
    FD_ZERO(&read_fds);
    FD_SET( (uint16_t) bip_socket(), &read_fds);
    max = bip_socket();
...
//    CheckLocalAddressKnown( portParams, &sin);
    ...
    {
        /// hmmmm, 2019.02.28 Steve's code depends on this. (distribute-broadcast-to-network). todo 0 - Investigate, in particular SS
//        return 0;
    }
    ...
    bip_ipAddr_port_to_bacnet_mac(&dummyDstMAC, bip_get_addr(), bip_get_port() );

    bip_ipAddr_port_to_bacnet_mac(&dummySrcMAC, sin.sin_addr.s_addr, sin.sin_port);
    ...
    case BVLC_FORWARDED_NPDU:
        ...
        // [BTC todo] add a test for 0 and ~0 to BTC
        ...
        dbTraffic(DBD_BBMD, DB_INFO, "BVLL: Received Forwarded-NPDU from %s:%d.",
        ...
        if (bvlc_bdt_member_mask_is_unicast(&sin)) {
            ...
            dbTraffic(DBD_BBMD, DB_INFO, "BVLL: Received unicast from BDT member, re-broadcasting locally to %s:%d.",
        ...
        dest.sin_port = original_sin.sin_port;
            bvlc_fdt_forward_npdu(&dest, max_npdu, &npdu[4 + 6], npdu_len, false);

        // todo1 - conflicts with above (? according to notes in full routing c++. Review)
        dbTraffic(DBD_BBMD, DB_INFO, "BVLL: Received Forwarded-NPDU from %s:%d.",
            inet_ntoa(dest.sin_addr), ntohs(dest.sin_port));
    ...
    case BVLC_DISTRIBUTE_BROADCAST_TO_NETWORK:
        dbTraffic(DBD_BBMD, DB_INFO, "BVLL: Received Distribute-Broadcast-to-Network from %s:%d.",
        ...
        /* Upon receipt of a BVLL Distribute-Broadcast-To-Network message
           ...
           attempt was unsuccessful */ // [btc todo3 - what of nodes w/o BBMDs? to they too return 0060?]
            ...
            bvlc_forward_npdu(&sin, max_npdu, &npdu[4], npdu_len);
            bvlc_bdt_forward_npdu(&sin, &npdu[4], npdu_len, false);
            bvlc_fdt_forward_npdu(&sin, max_npdu, &npdu[4], npdu_len, false);
            /* not an NPDU */
            npdu_len = 0;
        break;
    ...
    {
        char tstr[100];
        dbTraffic(DBD_BBMD, DB_INFO, "BVLL: Received Original-Unicast-NPDU from %s",
    ...
    case BVLC_ORIGINAL_BROADCAST_NPDU:
        {
            char tstr[100];
            dbTraffic(DBD_BBMD, DB_INFO, "BVLL: Received Original-Broadcast-NPDU from %s",
                IPAddr_Port_ToString(tstr, &sin.sin_addr, sin.sin_port));
            ...
            if (npdu_len < max_npdu) {
                ...
                bvlc_fdt_forward_npdu(&sin, max_npdu, &npdu[0], npdu_len, true);
            }
            else {
                /* ignore packets that are too large */
                npdu_len = 0;
    ...
    unsigned pdu_len)
{
    ...
    if ((dest->net == BACNET_BROADCAST_NETWORK) || (dest->mac_len == 0)) {
        ...
        dbTraffic(DBD_BVLL, DB_NORMAL_TRAFFIC, "BVLL: Sending Original-Unicast-NPDU.");
    } else {
        /* invalid address */
        return -1;
    }
    ...
    uint16_t received_bytes)
{
    ...
    case BVLC_FORWARDED_NPDU:
        ...
        This BVLL message is used in broadcast messages from a BBMD as well as in messages forwarded to registered foreign
        devices. It contains the source address of the original node, [BTC] or if NAT is being used, the address with which the original node is
        accessed, as well as the original BACnet NPDU.
         **/
        break ;
