    ...
    void)
{
    ...
    dbTraffic( DB_NOTE, "host: %s at %u.%u.%u.%u", host_name,
    ...
    char *ifname)
{
    ...
    // todo1 atexit( datalink_cleanup);

    // We have a few interfaces to worry about, and currently we dynamically set them when the first incoming packet to the corresponding port arrives
    if (ifname)
    {
    ...
    {
        // we really dont want to start a system with no interface identified, else we can't use the interface's IPADDR in the MAC address on initial I-Am, etc.
        panic();

        /* has address been set? */
        portParams->bipParams.nwoBroadcast_addr = INADDR_BROADCAST;
        portParams->bipParams.nwoLocal_addr = INADDR_ANY;
    }
...
//    }
    ...
    // }
    if (BIP_Debug) {
        broadcast_address.s_addr = portParams->bipParams.nwoBroadcast_addr ;
    ...
    portParams->bipParams.socket = sock_fd ;
    if (sock_fd < 0) {
    ...
    sin.sin_addr.s_addr = portParams->bipParams.nwoLocal_addr ;
#endif
    ...
    if (rv < 0) {
        fprintf(stderr, "bip: failed to bind to %s port %hu\n",
                inet_ntoa(sin.sin_addr), portParams->bipParams.nwoPort );
        closesocket(sock_fd);
        portParams->bipParams.socket = -1;
        return false;
    }
 ...
 * @ingroup DLBIP
  */
void bip_cleanup( PORT_SUPPORT *portParams )
{
    if ( portParams->socket >= 0 ) {
        closesocket(portParams->socket);
    }
    portParams->socket = -1;
    WSACleanup();
}
